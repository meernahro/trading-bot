...
class BybitSpotClient(ExchangeClientBase):
    ...
    def get_order(self, symbol: str, order_id: str) -> Dict:
        """Get order details"""
        try:
            response = self.client.get_order_history(category="spot", orderId=order_id)
    ...
    def get_order_book(self, symbol: str, limit: int = 100) -> Dict:
        ...
        try:
            ...
            return {
                "symbol": symbol,
                "bids": [
                    [float(price), float(qty)] for price, qty in response["result"]["b"]
                ],
                "asks": [
                    [float(price), float(qty)] for price, qty in response["result"]["a"]
                ],
